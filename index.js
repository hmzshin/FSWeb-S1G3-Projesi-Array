...
function kopyala() {
  let len = orijinalTatlar.length;
  console.log(len);
  let kopya = [];

  for (let i = 0; i < len; i++) {
    kopya[i] = orijinalTatlar[i];
  }
  return kopya;
}
...
let new_orijinalTatlar = kopyala().length;

function dizi25Cesitmi() {
  let value = true;
  if (kopyala().length == 25) {
    value;
  } else {
    value = false;
  }
...
function cesitEkle(yeni_liste, yeni_tad) {
  yeni_liste.unshift(yeni_tad);
  return yeni_liste;
}

console.log(cesitEkle(kopyala(), "Kakule"));
...
function sonCesitiKaldir(sonOgeyiKaldir) {
  sonOgeyiKaldir.pop();
  return sonOgeyiKaldir;
}

let yeni_ekli = cesitEkle(kopyala(), "Kakule");
sonCesitiKaldir(yeni_ekli);
...
function indekstekiCesitiGetir(tat, sayı) {
  let sonuc = tat[sayı];
  return sonuc;
}
...
function ismeGoreCesitCikar(tat1, cıkarılacak_malzeme) {
  let cıkan_malzeme = [];

  for (let i = 0; i < tat1.length; i++) {
    if (cıkarılacak_malzeme == tat1[i]) {
      cıkan_malzeme = tat1.splice(i, 1); //splice komutu o indexi çıkarır ve çıkarılan index ileyeni bir array oluşturur  bu yüzden return yaparken splice değilde main array  return edilir çünkü splice içerisinden istediğimiz değeri zaten çıkardı
    }
  }
  return tat1;
}

console.log(ismeGoreCesitCikar(kopyala(), "Vişne"));
/* Görev 7:
...
function ismeGoreFiltrele(tat2, filtrelenecek_tat) {
  let filtrelenen_malzeme = [];

  for (let i = 0; i < tat2.length; i++) {
    if (tat2[i].includes(filtrelenecek_tat)) {
      // .includes komutu true ya da false olarak değer döndürür.
      filtrelenen_malzeme.push(tat2[i]);
    }
  }
  return filtrelenen_malzeme;
}
